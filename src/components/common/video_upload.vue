  ...
  <div :style="getContainerStyle()" class="upload-container">
    ...
    <div class="img-container">
      <div v-for="img in uploadArray" :key="img.id" class="img">
        <!-- <div class="mask"></div> -->
        <!-- <img class="status-btn" src="@static/images/play.png" title="点击播放" @click="play(img)" > -->
        <div v-if="!img.uploading && mode !== 'readonly'" class="close" @click="close(img)">×</div>
        <img v-if="img.uploading" class="loading" src="@static/images/loading.gif">
        <video-player
      ...
      <div class="img add" :class="uploadArray.length >= maxCount || currentMode === 'readonly' ? 'hide' : ''" @click="toUpload">
        ...
        <div class="tip-container">
          <div class="to-add">点击添加视频</div>
          <div class="desc">
            <slot name="desc">
              <div>请录制拍摄制作beat或音乐的工程</div>
              <div>视频文件</div>
            </slot>
          </div>
        </div>
...
//        playbackRates: [0.7, 1.0, 1.5, 2.0], //播放速度
  ...
  methods: {
    ...
    onChange(e) {
      ...
      let file = files[0]
      if (file) {
        if (this.validateType && !/^video/.test(file.type)) {
          this.tipUtil.warning('请上传视频格式的文件!')
          this.form.reset()
          return
        }
        ...
        if (maxSize && file.size > maxSize) {
          this.tipUtil.warning(`视频大小不能超过${maxSize / 1024 / 1024}M`)
          return
        }
    ...
    preview(img) {
      this.$store.commit('showPreviewImg', this.pathUtil.getOssImagePath(img.path))
    },
    close(img) {
      if (img.uploading) {
        this.tipUtil.warning('正在上传中，请稍后再操作')
        return
      }
    ...
    toUpload() {
      ...
      if (this.uploadArray.length >= this.maxCount) {
        this.tipUtil.warning(`最多只能上传${this.maxCount}个视频`)
        return
      }
